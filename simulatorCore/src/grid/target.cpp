...
// vector<Cell3DPosition> obstacles = {}; //By Hussein
...
TargetGrid::TargetGrid(TiXmlNode *targetNode) : Target(targetNode) {
    ...
    while (cellNode) {
        ...
        // attr = element->Attribute("obstacle"); //By Hussein
	// if(attr){ //By Hussein
        attr = element->Attribute("color");
		color.set(attr);
		Cell3DPosition temPos = Cell3DPosition(position);
		// Target::obstacles.push_back(temPos);
        //getWorld()->lattice->unhighlightCell(temPos);
