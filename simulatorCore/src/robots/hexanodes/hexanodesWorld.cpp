...
#ifdef WIN32
    string directory = string(ROOT_DIR) + "/simulatorCore/resources/textures";
#else
    string directory = "../../simulatorCore/resources/textures";
#endif
...
HexanodesWorld::HexanodesWorld(const Cell3DPosition &gridSize, const Vector3D &gridScale,
    ...
    if (GlutContext::GUIisEnabled) {
        objBlock = new ObjLoader::ObjLoader((directory+"/hexanodesTextures").c_str(),"hexanodes.obj");
        objBlockForPicking = new ObjLoader::ObjLoader((directory+"/hexanodesTextures").c_str(),"hexanodes_picking.obj");
...
void HexanodesWorld::createPopupMenu(int ix, int iy) {
    if (!GlutContext::popupMenu) {
        ...
        addBlockSubMenu->addButton(11,(directory+"/menuTextures/menu_add_normal.tga").c_str());
        addBlockSubMenu->addButton(12,(directory+"/menuTextures/menu_add_same.tga").c_str());
        addBlockSubMenu->addButton(13,(directory+"/menuTextures/menu_add_random.tga").c_str());
        ...
        GlutContext::popupMenu->addButton(1,(directory+"/menuTextures/menu_add_sub.tga").c_str(),addBlockSubMenu);
        GlutContext::popupMenu->addButton(2,(directory+"/menuTextures/menu_del.tga").c_str());
        GlutContext::popupMenu->addButton(6,(directory+"/menuTextures/menu_rotate_sub.tga").c_str(),rotateBlockSubMenu);
        GlutContext::popupMenu->addButton(3,(directory+"/menuTextures/menu_tap.tga").c_str());
        GlutContext::popupMenu->addButton(4,(directory+"/menuTextures/menu_save.tga").c_str());
        GlutContext::popupMenu->addButton(5,(directory+"/menuTextures/menu_cancel.tga").c_str());
    }
    ...
    if (nbreMenus==0) {
        ...
        for(auto &motion:tab) {
            ...
            rotateBlockSubMenu->addButton(new GlutRotation2DButton(NULL,i++,0,0,0,0,(directory+"/menuTextures/menu_link_hexanodes.tga").c_str(), motion->direction==CCW,motion->fromConId,idDest,finalPos,orient,0.083333333));
        }
    }
...
void HexanodesWorld::addObstacle(bID blockId, BlockCodeBuilder bcb, const Cell3DPosition &pos, const Color &col, short orientation){
