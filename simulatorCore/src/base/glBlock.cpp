...
GlBlock::GlBlock(bID id,const Vector3D &pos, const Color &col) : blockId(id) {
    position[0] = pos[0];
    position[1] = pos[1];
    position[2] = pos[2];
...
void GlBlock::setColor(const Color &col) {
    color[0] = GLubyte(col[0]);
    color[1] = GLubyte(col[1]);
    color[2] = GLubyte(col[2]);
}
...
string GlBlock::getInfo() {
    ...
    Color bcolor(color[0],color[1],color[2]);
    out << "Col=" << bcolor.getName();
