 ...
 *  @authors{Benoît Piranda: benoit.piranda@femto-st.fr, Pierre Thalamy: pierre.thalamy@femto-st.fr, Dominique Dhoutaut: dominique.dhoutaut@femto-st.fr}
 */
...
namespace BaseSimulator {
    ...
    World::World(int argc, char *argv[]) {
        ...
        if (world == nullptr) {
            ...
            ERRPUT << TermColor::ErrorColor << "Only one World instance can be created, aborting !" << TermColor::Reset
                   << endl;
            exit(EXIT_FAILURE);
        }
    }
...
#ifdef WIN32
        GlutContext::helpWindow = new GlutHelpWindow(nullptr, 10, 40, 540, 500, (string(ROOT_DIR) +
                                                                                 "/simulatorCore/resources/help/genericHelp.txt").c_str());
#else
...
#ifdef WIN32
        static string dir = string(ROOT_DIR) + "/simulatorCore/resources/textures/menuTextures/";
#else
        static string dir="../../simulatorCore/resources/textures/menuTextures/";
#endif
        if (!GlutContext::popupMenu) {
            GlutContext::popupMenu = new GlutPopupMenuWindow(nullptr, 0, 0, 200, 180);
            GlutContext::popupMenu->addButton(1, (dir + "menu_add.tga").c_str());
            GlutContext::popupMenu->addButton(2, (dir + "menu_del.tga").c_str());
            GlutContext::popupMenu->addButton(3, (dir + "menu_tap.tga").c_str());
            GlutContext::popupMenu->addButton(4, (dir + "menu_save.tga").c_str());
            GlutContext::popupMenu->addButton(5, (dir + "menu_cancel.tga").c_str());
        }
