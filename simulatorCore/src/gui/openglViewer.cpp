...
void GlutContext::init(int argc, char **argv) {
    if (GUIisEnabled) {
        ...
        mainWindow = new GlutSlidingMainWindow(screenWidth-40,60,40,screenHeight-60,
                                               (string(ROOT_DIR)+"/simulatorCore/resources/textures/UITextures/fenetre_onglet.tga").c_str());
#else
        mainWindow = new GlutSlidingMainWindow(screenWidth-40,60,40,screenHeight-60,
...
//          case 'p' : BlinkyBlocks::getDebugger()->handlePauseRequest(); break;
        ...
        default: { // Pass on key press to user blockcode handler
            ...
            //  this instance
            BuildingBlock *bb = BaseSimulator::getWorld()->getSelectedBuildingBlock() ?BaseSimulator::getWorld()->getSelectedBuildingBlock():BaseSimulator::getWorld()->getMap().begin()->second;
